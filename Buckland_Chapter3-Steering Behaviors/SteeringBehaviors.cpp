...
{
  if (!IsHunterOnVisionRange(hunter->Pos()) && !IsHunterClose()) { return Vector2D(0, 0); }
  double    DistToClosest = MaxDouble;
  Vector2D  BestHidingSpot;
  SetHidingDelimiters(hunter);
...
{
    double distanceFromHunter = m_pVehicle->Pos().Distance(posHunter);
    if (distanceFromHunter <= VisionRangeRad) {
        m_dLasTimeSeen = m_ptTimer.CurrentTime();
        return true;
    }
    return false;
}
...
//------------------------------------------------------------------------
bool SteeringBehavior::IsHunterClose()
{
    if (m_dLasTimeSeen > 0.00000001) {
        double elpasedSeconds = m_ptTimer.CurrentTime() - m_dLasTimeSeen;
        bool isClose = elpasedSeconds <= SecondsThreat;
        if (isClose) { auxHunterDistance = ttos(elpasedSeconds); }
        return isClose;
    }
    return true;
}
...
{
    Vector2D localPoint = PointToLocalSpace(posOb,
                                            hunter->Heading(),
                                            hunter->Side(),
                                            hunter->Pos());
    if (localPoint.x <= 0) {
        // the hinding spot is behind the hunter
        bestHiddingSpots.push_back(posOb);
        return true;
    }
    localPoint.Normalize();
    double auxDotValue = fabs(localPoint.Dot(Vector2D(1,0)));
    if (auxDotValue <= 0.87) {
        // acos(0.87) = 30 degs
        bestHiddingSpots.push_back(posOb);
        return true;
...
void SteeringBehavior::SetHidingDelimiters(const Vehicle* hunter)
{
    ...
    Vector2D heading = Vector2D(hunter->Heading().x, hunter->Heading().y);
    Vector2D leftDelimiter = Vector2D(heading.x, heading.y);
    Vector2D rightDelimiter = Vector2D(heading.x, heading.y);
    Vec2DRotateAroundOrigin(leftDelimiter, 0.52);
    Vec2DRotateAroundOrigin(rightDelimiter, -0.52);
    vertices.push_back(Vector2D(0, 0));
    vertices.push_back(leftDelimiter * VisionRangeRad);
    vertices.push_back(rightDelimiter * VisionRangeRad);
    m_HidingSpotDelimiter = CustomWorldTransform(vertices, hunter->Pos());
}
...
/////////////////////////////////////////////////////
  ...
  if (On(hide) && m_pVehicle->World()->RenderVisionRangeCircle())
  {
      ...
      if (m_pVehicle->ID() == 0)
      { 
          gdi->TextAtPos(5, NextSlot, "Extension after seen:"); gdi->TextAtPos(160, NextSlot, auxHunterDistance); NextSlot += SlotSize;
          gdi->TextAtPos(5, NextSlot, "Time Elapsed:"); gdi->TextAtPos(160, NextSlot, ttos(m_ptTimer.CurrentTime())); NextSlot += SlotSize;
          gdi->TextAtPos(5, NextSlot, "Last Time Seen:"); gdi->TextAtPos(160, NextSlot, ttos(m_dLasTimeSeen)); NextSlot += SlotSize;
          if (!posibleHiddingSpots.empty()) {
              std::vector<Vector2D>::const_iterator curSpot = posibleHiddingSpots.begin();
              while (curSpot != posibleHiddingSpots.end()) {
                  gdi->RedPen();
                  gdi->Circle(*curSpot, 1.0);
                  ++curSpot;
              }
              posibleHiddingSpots.clear();
              curSpot = bestHiddingSpots.begin();
              gdi->DarkGreenBrush();
              while (curSpot != bestHiddingSpots.end()) {
                  gdi->DarkGreenPen();
                  gdi->Circle(*curSpot, 2.0);
                  ++curSpot;
              }
              gdi->HollowBrush();
              bestHiddingSpots.clear();
          }
          if (!m_HidingSpotDelimiter.empty()) {
              gdi->LightGreyPen();
              gdi->Line(m_HidingSpotDelimiter.at(0), m_HidingSpotDelimiter.at(1));
              gdi->Line(m_HidingSpotDelimiter.at(0), m_HidingSpotDelimiter.at(2));
              m_HidingSpotDelimiter.clear();
          }
      }
  }
  //--------------------------------------------------------------------------------------------------------------------------------------------
