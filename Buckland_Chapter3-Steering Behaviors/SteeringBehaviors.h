...
//the radius of the vision Range of the pray for Hide behavior
const double VisionRangeRad = 140;
//the seconds of latent threat
const double SecondsThreat = 2.0;
...
private:
  ...
  //Timer
  PrecisionTimer        m_ptTimer;

  //the current position on the wander circle the agent is
  //attempting to steer towards
  Vector2D     m_vWanderTarget;

  //vertex that delimits the hiding spots
  std::vector<Vector2D> m_HidingSpotDelimiter;

  //explained above
  double        m_dWanderJitter;
  double        m_dWanderRadius;
  double        m_dWanderDistance;
  double        m_dLasTimeSeen;
  ...
  //auxiliar variables for debugging
  std::string auxHunterDistance = "";
  std::vector<Vector2D> posibleHiddingSpots;
  std::vector<Vector2D> bestHiddingSpots;
